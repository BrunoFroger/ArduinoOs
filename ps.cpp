...
int ps_exec(taskStruct *task){
    ...
    sprintf(printString, "+  id +                                          name      +  pid  +  ppid + prio+ curr+status+\n"); Serial.print(printString);
    sprintf(printString, "+-----+----------------------------------------------------+-------+-------+-----+-----+------+\n"); Serial.print(printString);
    for (int i = 0 ; i< NB_TASKS ; i++){
        ps_task=taskTbl[i];
        if (ps_task.status != FREE){
            sprintf(printString, "+ %3d + %50s + %5d + %5d + %3d + %3d + %4s +\n",i, ps_task.name, ps_task.pid, ps_task.ppid, ps_task.priority, ps_task.currentPriority, status2string(ps_task.status)); Serial.print(printString);
        }
    }
    sprintf(printString, "+-----+----------------------------------------------------+-------+-------+-----+-----+------+\n"); Serial.print(printString);
